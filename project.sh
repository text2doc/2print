...
current_version=$(grep -o "__version__ = version = '[0-9.]*'" src/2print/_version.py | grep -o "[0-9.]*")
...
cat > src/2print/_version.py << EOF
...
echo ""
git add src/2print/_version.py setup.py "$changelog_file" increment_init.py increment_setup.py
...
rm -rf build/ dist/ src/2print.egg-info/
